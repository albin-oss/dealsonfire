...
describe('manifest completeness', () => {
  ...
  it('the REAL manifest covers every kernel table with complete fields', () => {
    ...
    expect(manifestTableNames().size).toBe(55) // +5 operations (OPS-001A) +11 identity (R1-B1) +1 onboarding (MER-002) +2 catalog attrs (PROMPT-016) +1 media (UX-AUTHOR) +1 listings (CS1) +1 deals (R0.3) +3 engagement (R0.4) +2 sparks (R0.6) +6 orders (Commerce Foundation C1: quartet + carts + cart_lines)
  })
})
...
describe('immutability grants are represented in deploy SQL (ADR-004 C4)', () => {
  it('the grants file revokes UPDATE/DELETE and grants only SELECT/INSERT on both tables', async () => {
    ...
    expect(sql).toMatch(/REVOKE UPDATE, DELETE, TRUNCATE ON audit_logs, domain_events, commerce_audit_logs, commerce_domain_events, operations_audit_logs, operations_domain_events, identity_audit_logs, identity_domain_events, orders_audit_logs, orders_domain_events FROM \{\{APP_ROLE\}\}/)
    expect(sql).toMatch(/GRANT SELECT, INSERT ON audit_logs, domain_events, commerce_audit_logs, commerce_domain_events, operations_audit_logs, operations_domain_events, identity_audit_logs, identity_domain_events, orders_audit_logs, orders_domain_events TO \{\{APP_ROLE\}\}/)
