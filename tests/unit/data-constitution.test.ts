...
describe('manifest completeness', () => {
  ...
  it('the REAL manifest covers every kernel table with complete fields', () => {
    ...
    expect(manifestTableNames().size).toBe(82) // +5 operations (OPS-001A) +11 identity (R1-B1) +1 onboarding (MER-002) +2 catalog attrs (PROMPT-016) +1 media (UX-AUTHOR) +1 listings (CS1) +1 deals (R0.3) +3 engagement (R0.4) +2 sparks (R0.6) +6 orders (Commerce Foundation C1) +3 stock (C2) +5 checkout (C3) +10 payments (C4) +3 fulfillment (C6) +2 returns (C9) +1 boundary journal +3 disputes-reconciliation (C10)
  })
})
