/** POST /api/v1/products/:productId/unpublish-from-store (VISIBILITY_CONTRACT §6). */
...
export default defineCommandEndpoint({
  command: 'commerce.listing.unpublish',
  ...
  async handler({ event, auth, body, requestContext }): Promise<Result<{ status: string; changed: boolean }, DomainError>> {
    ...
    const result = await getContainer().commerce.commands.unpublishFromStore({
