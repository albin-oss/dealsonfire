...
describe('selectOpportunity — momentum (Release 0.8)', () => {
  ...
  it('quiet store with followers → say good morning', () => {
    const progress = { ...late, momentum: { followers: 14, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: 72, unsparked_product: null, attention_this_week: null } }
  ...
  it('never-published store with followers → good morning (hours_quiet null)', () => {
    const progress = { ...late, momentum: { followers: 1, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: null, unsparked_product: null, attention_this_week: null } }
    const opp = selectOpportunity(progress)
    expect(opp.id).toBe('morning-spark')
  ...
  it('recently active → the unsparked product gets its moment (one-click hand-off)', () => {
    const progress = { ...late, momentum: { followers: 14, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: 3, unsparked_product: { id: 'p-1', title: 'Wool Scarf' }, attention_this_week: null } }
    const opp = selectOpportunity(progress)
  ...
  it('no followers, nothing unsparked → share-store stands', () => {
    const progress = { ...late, momentum: { followers: 0, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: null, unsparked_product: null, attention_this_week: null } }
  ...
  it('outranks the parked ladder: first_product + real momentum facts → spark, not "add a product"', () => {
    const progress = { ...progressAt('first_product'), momentum: { followers: 2, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: 1, unsparked_product: { id: 'p-9', title: 'Pen' }, attention_this_week: null } }
    const opp = selectOpportunity(progress)
    expect(opp.id).toBe('spark-product')
  ...
  it('first_product with NO momentum facts keeps the shelf prompt', () => {
    const progress = { ...progressAt('first_product'), momentum: { followers: 0, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: null, unsparked_product: null, attention_this_week: null } }
    expect(selectOpportunity(progress).id).toBe('first-product')
  })

  it('quiet beats unsparked: the audience waits first', () => {
    const progress = { ...late, momentum: { followers: 5, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: 96, unsparked_product: { id: 'p-1', title: 'Wool Scarf' }, attention_this_week: null } }
...
describe('pulseSentence — the feedback loop (Release 1.2)', () => {
  ...
  const withMomentum = (m: Partial<NonNullable<OnboardingProgressResponse['momentum']>>) => ({
    ...base,
    momentum: { followers: 0, fires_this_week: 0, new_followers_this_week: 0, products_on_store: 0, live_deals: 0, sparks_published: 0, recent_activity: [], hours_quiet: null, unsparked_product: null, attention_this_week: null, ...m },
  })
