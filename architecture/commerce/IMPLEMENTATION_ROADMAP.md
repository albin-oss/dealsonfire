...
| **C5** | **Confirmation & the merchant's first sale** — commit-with-race handling (honest re-offer, pre-capture), **the single full capture at `confirmed`** (partial only when the race dropped a line; `payments.hold.opened`), the new-order Pulse task, first-sale Signature Moment, buyer order history + timeline page, `payment_pending/payment_failed` retry window *(AMENDMENT-001 A1/A5)* | 4 | C4 | The last-unit race on stage: two buyers, one honest re-offer; the merchant's first-sale moment fires with real money in the balance |
| **C6** | **Fulfillment & shipping v1** — ShippingProfile/Zone/Rate (flat + free-over + pickup) with Ignite Reveal wiring (OPS-002 gap); `ShippingQuoteQuery` in checkout; FulfillmentCase per (location, method); merchant pack/dispatch flow with manual tracking; line states → `partially_fulfilled/fulfilled`; **hold-release postings + `payments.hold.released` on fulfillment evidence** *(AMENDMENT-001 A6 — capture-on-fulfillment removed)* | 5 | C5 | Merchant packs 2 of 3 lines; buyer timeline says so honestly; the packed lines' money becomes payout-eligible |
...
| **C12** | **Marketplace money hardening** — **HoldPolicy live** (trust-tiered payout delay + release sweeps; no-ship aging → auto-refund path), payout scheduling via the Payouts API, dispute aggregate + reserve freezes + evidence assembler + deadline tasks, reconciliation hardening incl. the balance identity check (statement ingestion, discrepancy SLA); flash-sale load test; **GA gate: legal/compliance review + PCI SAQ-A attestation (ADR-008 R-c)** *(AMENDMENT-001 A4/A7)* | 5 | C4–C9 | Held funds release on verification; a dispute arrives with evidence 80% assembled; the drift alarm demo |

**Total: ~51 engineering days** to a GA-gated, marketplace-grade commerce foundation.

**AMENDMENT-001 (2026-07-26):** C3–C12 corrected for the capture-at-confirmation policy (PAYMENT_POLICY_DECISION.md). C3 and C6 shrank (no fulfillment-triggered captures); C4 and C5 grew (onboarding shape; capture at confirm + facts-register re-verification against the pinned Stripe API version). Net schedule change ≈ 0 — **the ~51-day estimate remains credible.** C4 additionally pins the Stripe API version as its first commit.
