...
| **SV_1_CERTIFICATION** | **CLOSURE** — The Maker Controls Their Own Store (GO, v1.55.0): pause/close/restore activate the merchant-owned edges of the frozen ADR-001 §7 machine (status ⊥ enforcement_hold preserved; paused carries a reason; Closed ≠ Deleted, reversible 90 days). ZERO schema change; the one `status='live'` visibility law carries it to every public surface; existing orders/money untouched; owner-only + step-up + audit on close/restore; 13-scenario hostile matrix |
| **SV_2_CERTIFICATION** | **CLOSURE** — The Maker Shapes Their Store (GO, v1.56.0): Store Settings on `/store` (identity/appearance/story/policies/handle) per ADR §11's content list. Handle change is immutable-with-redirect (owner-only + step-up + audit; old links never 404, old addresses un-hijackable, A→B→C chains flattened); logo now resolves + renders (with a cross-tenant guard); display name syncs to stores.name; returns copy is DOF's derived 30-day truth, never merchant free-text. ZERO schema change; propagation is free (live joins). Inherited SV-1 browser gap fixed (corrupt dev data + dev-only ?dev-user); 15-scenario hostile matrix, browser-proven |
| **EP_1_CERTIFICATION** · **CURRENT_PRICE_TRUTH_MAP** | **CLOSURE** — One Price, Everywhere (GO, v1.58.0): the Effective Price Foundation. ONE `EffectivePriceService` (ADR-002 §8/D2-7) resolving base → active (window-checked) sale, used by storefront/preview/checkout so display and charge can't diverge; ~20 copy-pasted min-price SQL sites + 4 window-blind charge sites collapsed to one rule (TS resolver for charge + canonical SQL fragment for display, drift-guarded). FIXES a live money bug (expired/future sales were charged); enforces single-currency-per-order. ZERO schema/events; order/Stripe/refund/fee/shipping untouched. Offer insertion point marked; CS4 safe to begin next |
| **SV_3_CERTIFICATION** | **CLOSURE** — The Maker Runs Their Store (GO, v1.57.0): Inventory/Shipping/Returns finished as real Workspace destinations over the existing Operations backends. New inventory adjust (set/±delta → existing ledger under the reserve lock, turns tracking on, reason-coded/audited, `catalog.inventory.write`, can't set below in-flight reservations) + a catalog-driven read; shipping UI on the existing profile (one truth; `/store` demoted to a door); a returns queue projecting the four-state machine (actions door to Orders, no PII); honest enforcement-hold notice. ZERO schema/events; money & oversell laws untouched. 11-scenario hostile matrix, merchant day browser-walked desktop+mobile |
| **STORE_V1_CLOSURE** | **CLOSURE (critical)** — Store v1 judged COMPLETE (GO) against the whole maker journey (idea→Ignite→identity→lifecycle→inventory→shipping→fulfil→return→demand); candid register of important non-blockers (low-stock nudge, buyer delivered-signal, refund gating, keyset paging) and the SV-1→SV-3 debt/watch-triggers; roadmap re-entry = Offers → Deals landing → Flicks |
