/** DELETE /api/v1/products/:productId/options/:optionName — remove an unused option. */
...
export default defineCommandEndpoint({
  command: 'commerce.product.option_remove',
  ...
  async handler({ event, auth, requestContext }): Promise<Result<ProductDTO, DomainError>> {
    ...
    const result = await getContainer().commerce.commands.removeOption({
